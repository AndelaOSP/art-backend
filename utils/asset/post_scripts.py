...
def post_data(file):
    ...
    # file_length = sum(1 for row in data.reader)
    ...
    for row in data:
        ...
        assetcode_value = row.get('Serial No.', '').strip()
        category_object = collection_bootstrap(AssetCategory, category_name=category_value)
        subcategory_value = {'sub_category_name': subcategory_value, 'asset_category': category_object}
        subcategory_object = collection_bootstrap(AssetSubCategory, **subcategory_value)
        # subcategory_value = {'sub_category_name': subcategory_value, 'asset_category': category_object}
        # collection_bootstrap(AssetSubCategory, category_name=category)
        # collection_bootstrap(AssetCategory, category_name=category)
        # collection_bootstrap(AssetCategory, category_name=category)
