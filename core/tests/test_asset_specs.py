...
from ..models import AssetSpecs, Asset
...
class AssetModelNumberTestCase(CoreBaseTestCase):
    def test_can_create_new_asset_specs(self):
        ...
        asset_specs = AssetSpecs.objects.create(
            ...
            processor_type="Intel core i5", memory=16, storage=512,
        )
        self.assertEqual(AssetSpecs.objects.count(), count + 1)

        new_asset = Asset.objects.create(
            asset_code="IC010", serial_number="SN00450",
            model_number=self.test_assetmodel, purchase_date="2018-07-10",
            assigned_to=self.asset_assignee, specs=asset_specs,
        )
    ...
    def test_should_not_create_field_with_invalid_choices(self):
        with self.assertRaises(ValidationError):
            AssetSpecs.objects.create(
                screen_size=19, year_of_manufacture=2011, processor_speed=3.0,
                processor_type="Intel core i5", memory=16, storage=512
            )
