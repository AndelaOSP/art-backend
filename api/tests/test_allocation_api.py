...
class AllocationTestCase(APIBaseTestCase):
    ...
    def test_post_allocation_of_asset_to_a_user(self, mock_verify_id_token):
        ...
        response = client.post(self.allocations_urls, data, HTTP_AUTHORIZATION="Token {}". format(self.token_user))
        self.assertEqual(AllocationHistory.objects.count(), count + 1)
        self.assertEqual(response.data['asset'], f"{self.asset.serial_number} - {self.asset.asset_code}")
