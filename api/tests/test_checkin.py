...
from ..models import Checkin, ItemModelNumber, Item, SecurityUser
...
class CheckinModelTest(TestCase):
    ...
    def test_non_authenticated_user_checkin(self):
        response = client.get(self.checkin_url)
    ...
    def test_authenticated_normal_user_list_checkin(
    ...
    @patch('api.authentication.auth.verify_id_token')
    def test_authenticated_security_user_list_checkin(
            self, mock_verify_id_token):
        ...
        self.assertEqual(len(response.data), Checkin.objects.count())
    ...
    def test_authenticated_security_user_view_checkin_detail(
        ...
        mock_verify_id_token.return_value = {'email': self.security_user.email}
        response = client.get(
            "{}{}/".format(self.checkin_url, self.checkin.id),
            HTTP_AUTHORIZATION="Token {}".format(self.token_security_user))
