...
class UserFeedbackModelTest(TestCase):
    ...
    def test_can_save_feedback(self):
        UserFeedback.objects.create(reported_by = "hawi.Y@andela.com", message = "This is feedback", report_type = "bug")
        new_feedback = UserFeedback.objects.get(reported_by = "hawi.Y@andela.com")
        new_feedback_count = UserFeedback.objects.count()
        

        self.assertEqual(new_feedback_count, 2)
        self.assertIn(new_feedback.reported_by,"hawi.Y@andela.com")